...
To do this:
1. Press <Kbd keys={["command"]}>\`</Kbd> or <Kbd keys={["ctrl"]}>\`</Kbd> or click `View > Terminal` to open the terminal.
...
1. Press <Kbd keys={["command", "shift"]}>P</Kbd> or <Kbd keys={["ctrl", "shift"]}>P</Kbd> or click `View > Command Palette` to open the **Command Palette** and search for `git: Clone`, once it is selected, press enter.
...
1. Open the terminal with <Kbd keys={["command"]}>\`</Kbd> or <Kbd keys={["ctrl"]}>\`</Kbd> or by clicking `View > Terminal`.
