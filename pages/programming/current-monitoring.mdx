# Current Spike Detection & Monitoring Current
...
A motor stall occurs when a motor is held stationary, but is requested to move.
This results in the motor drawing a large amount of current in an attempt to move.
Current draw in the motor is exponentially proportional to heat as shown by the equation:
$\text{Heat}=I^{2}*R$ Heat is bad for motors, when a motor gets hot it becomes less efficient,
if a motor becomes too hot, it may become damaged;
such as the motor windings melting together causing a short.

Below is an image of a simulated stall using the WPILib simulator, where you can see,
